...
K.basePath = "http://"+window.location.host+'/kindeditor-4.1.10/style/kindeditor/';
K.options = {
	designMode : true,
	...
	zIndex : 811213,
	items : [
	    'undo', 'redo','forecolor', 'bold',
        'italic','removeformat', 'justifyleft', 'justifycenter', 'justifyright',
        'justifyfull',"image"
	...
	colorTable : [
	    ['#292a2e','#4e4e4e', '#888888','#9da1a7 ', '#ff3234', '#ffa132','#5c83b2']
	...
	htmlTags : {
		span : [
			'.color', '.font-weight', '.font-style', '.vertical-align', '.text-align','max-width',
		],
		img : ['src', 'width', 'height', 'alt', 'title', 'align', '.width', '.height', '.border','max-width',
		],
		'p,strong,br' : [
			 'align', '.text-align', '.color', '.font-size','.font-weight', '.font-style', '.vertical-align', 'max-width',
		],
		/*font : ['id', 'class', 'color', 'size', 'face', '.background-color'],
		span : [
			'id', 'class', '.color', '.font-size',
...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {
		//console.log($0+"_0_"+ $1+"_1_"+$2+"_2_"+$3+"_3_"+$4+"_4_"+$5+"_2_"+$6);
		var full = $0,
			startNewline = $1 || '',
			startSlash = $2 || '',
		...
		if (attr !== '') {
			...
			_each(attrMap, function(key, val) {
				...
				if (key === 'style' && val !== '') {
					...
					/**********图片限制最大为100%**************/
					if(tagName=="img"){
						attrMap.style = style+'max-width:100%;';
					}else{
						attrMap.style = style;
					}
				}
...
_extend(KCmd, {
	...
	showBlankTip:function(){
		var self = this;
		var input_detil=$('.inpt-detil');
		if( input_detil.length>0){
			input_detil.removeAttr("style");
			input_detil.prev().children(".warm-tip").remove();
		}
		var kindeditor_tip=$('#kindeditor-tip');
		if(kindeditor_tip.length>0){
			kindeditor_tip.hide();
		}
	},
	hr : function() {
...
_extend(KWidget, {
	init : function(options) {
		...
		if(options.cls&&options.cls=="ke-dialog-mask")
		{
			self.div.css('position', 'fixed');
		}
...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	var arr = [
		...
		'body {margin:0;padding:20px 60px;}',
		'body, td {font:14px/1.5 "Microsoft YaHei",tahoma,verdana,helvetica;}',
	...
	showBlankTip:function(){
		...
		if( input_detil.length>0){
			if(self.html().length==0){
				if(input_detil.prev().find('.warm-tip').length==0){
					input_detil.attr('style','border-color:#46c8a4');
					input_detil.prev().append('<span class="warm-tip">( 同学，你总得记录一下发生什么吧。)</span>');
				}
			}else{
				input_detil.removeAttr("style");
				input_detil.prev().children(".warm-tip").remove();
...
_extend(KDialog, KWidget, {
	...
	setMaskIndex : function(z) {
		...
		//self.mask.div.css('position', 'fixed');
	},
...
_plugin('core', function(K) {
	...
	self.clickToolbar('fontsize', function() {
		...
		_each(self.fontSizeTable, function(i, val) {
			...
			menu.addItem({
				title : '<span style="font-size:' + val + ';" unselectable="on">' + valName + '</span>',
